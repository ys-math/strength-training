...
const straightOf = (rows: SetRow[], lift: LiftKey) => prescribe(rows, lift).plan
...
describe('bandOf — labels and clamps', () => {
  ...
  // Doubles and singles are real sessions, but there is no band below heavy, so they clamp
  // to it rather than falling out of the cycle.
...
describe('liftDays — reading a messy log', () => {
  ...
  it('keeps the modal load as the straight sets when a heavier set was logged', () => {
    const rows = [...day('2026-09-08', 'BP', 60, 10), set('2026-09-08', 'BP', 70, 2)]
    const [d] = liftDays(rows, 'BP')
    expect(d.load).toBe(60)
    expect(d.reps).toEqual([10, 10, 10])
    expect(d.band).toBe('volume')
  })

  // What the trend chart plots. It falls back to the working load so every trained session
  // has a point, which matters most for overhead press: 30 sessions, few heavier singles.
  it('always has a heaviest working set, heavier single or not', () => {
    const withHeavier = [...day('2026-09-08', 'BP', 60, 10), set('2026-09-08', 'BP', 70, 2)]
    expect(liftDays(withHeavier, 'BP')[0].heaviest).toEqual({ weight: 70, reps: 2 })

    const without = day('2026-09-08', 'SQ', 60, 5)
    expect(liftDays(without, 'SQ')[0].heaviest).toEqual({ weight: 60, reps: 5 })
  })

  it('finds a heaviest set that is not the modal load', () => {
