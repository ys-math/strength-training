...
import { e1rmSeries, type E1rmPoint } from '../lib/metrics'
...
function makeEndLabel(lastIndex: number, color: string, text: string) {
  return function EndLabel(props: { x?: number; y?: number; index?: number }) {
    ...
    return (
      <text x={props.x + 8} y={props.y} dy={4} fontSize={11} fontWeight={700} fill={color}>
        {text}
      </text>
    )
  }
...
export default function E1RMChart({ rows }: { rows: SetRow[] }) {
  const data = useMemo(() => e1rmSeries(rows), [rows])
  ...
  return (
    <ChartCard
      ...
      subtitle="Epley formula · best working set per session"
      right={legend}
    >
            ...
            {LIFTS.map((lift) =>
              hidden.has(lift.key) ? null : (
                <Line
                  ...
                  label={makeEndLabel(lastIndex[lift.key], lift.color, lift.key) as never}
                />
              ),
            )}
          </LineChart>
